...
//   return arr;
// }
const stringsToNumbs = numbers => numbers.map(Number);

const total = stringsToNumbs(strNums);
// console.log(total);
...
for (let i = 0; i < total.length; i++) {
  if(total[i] %2 === 0) {
    allEvens.push(total[i])
  }
}
const reducer = (accumulator, currentValue) => accumulator + currentValue;
// console.log(allEvens.reduce(reducer));

// Find the index of the first value when added to it's index = 512 (#ATX!!)
const atxIdx = total.findIndex((currentValue, index) => currentValue + index === 512)

// console.log(`index: ${atxIdx}, value: ${total[atxIdx]}`);
...
// const idealTemp = weather.filter(min => {
//   return min.min_temp === 15.915;
// })
//   .map(min => {
//     return min.id;
//   }) 
const idealTemp = weather.filter(min => min.min_temp === 15.915).map(min => min.id) 
    // const temp = idealTemp.map(min => min.id) 
